...
const AdminProfile = () => {
  ...
  useEffect(() => {
    if (adminId) {
      API.getById(adminId)
        .then((res) => {
          const adminData = res.data.admin || res.data; // support both { admin: {…} } and {…}
          const initialForm = {
            name: adminData.name || "",
  ...
  const handleSave = () => {
    if (validateForm()) {
      ...
      API.update(adminId, updatedData)
        .then(() => {
          const updatedAdmin = {
            ...form,
            password: "",
        ...
        <div className="mt-7 bg-white rounded-xl shadow-lg border-2 border-indigo-300">
          <div className="p-4 sm:p-7">
            <div className="text-center">
              <h1 className="block text-2xl font-bold text-gray-800">👤 Admin Profile</h1>
              <p className="mt-2 text-sm text-gray-600">Manage admin information and credentials here.</p>
            </div>
